...
namespace FEM {
    ...
    public:
        CauchyBC(const std::string& variable_name, const std::string& boundary_name, TScalar h_val, TScalar T_inf_val)
            : BoundaryCondition<TDim, TScalar>(variable_name, boundary_name, BCType::Cauchy), h_(h_val), T_inf_(T_inf_val) {}
        ...
        void apply(const Geometry& geometry, const DofManager& dof_manager,
            ...
            for (const auto& elem_ptr : boundary_elements) {
                ...
                for (size_t i = 0; i < face_element.getNumNodes(); ++i) {
                    int global_i = dof_manager.getNodeDof(this->getVariableName(), face_element.getNodeId(i), 0);
                    F_global(global_i) += F_elem_bc(i);
                    for (size_t j = 0; j < face_element.getNumNodes(); ++j) {
                        int global_j = dof_manager.getNodeDof(this->getVariableName(), face_element.getNodeId(j), 0);
