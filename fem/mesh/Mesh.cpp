...
namespace FEM {
    ...
    std::unique_ptr<Geometry> Mesh::create_uniform_2d_mesh(double width, double height, int nx, int ny) {
        ...
        for (int i = 0; i < nx; ++i) {
            int n0_id = i;
            int n1_id = i + 1;
            bottom_bnd->addElement(std::make_unique<LineElement>(edge_id_counter++, std::vector<Node*>{
                geometry->getMesh().getNodeById(n0_id),
                geometry->getMesh().getNodeById(n1_id)
            }));
        }
        ...
        for (int i = 0; i < nx; ++i) {
            int n0_id = ny * (nx + 1) + i;
            int n1_id = ny * (nx + 1) + i + 1;
            top_bnd->addElement(std::make_unique<LineElement>(edge_id_counter++, std::vector<Node*>{
                geometry->getMesh().getNodeById(n0_id),
                geometry->getMesh().getNodeById(n1_id)
            }));
        }
        ...
        for (int j = 0; j < ny; ++j) {
            int n0_id = j * (nx + 1);
            int n1_id = (j + 1) * (nx + 1);
            left_bnd->addElement(std::make_unique<LineElement>(edge_id_counter++, std::vector<Node*>{
                geometry->getMesh().getNodeById(n0_id),
                geometry->getMesh().getNodeById(n1_id)
            }));
        }
        ...
        for (int j = 0; j < ny; ++j) {
            int n0_id = j * (nx + 1) + nx;
            int n1_id = (j + 1) * (nx + 1) + nx;
            right_bnd->addElement(std::make_unique<LineElement>(edge_id_counter++, std::vector<Node*>{
                geometry->getMesh().getNodeById(n0_id),
                geometry->getMesh().getNodeById(n1_id)
            }));
        }
    ...
    std::unique_ptr<Geometry> Mesh::create_uniform_3d_mesh(double width, double height, double depth, int nx, int ny, int nz) {
        ...
        for (int k = 0; k < nz; ++k) {
            for (int j = 0; j < ny; ++j) {
                // left face (x=0) - 创建四边形面单元
                int n0_id = k * (nx + 1) * (ny + 1) + j * (nx + 1);
                int n1_id = k * (nx + 1) * (ny + 1) + (j + 1) * (nx + 1);
                int n2_id = (k + 1) * (nx + 1) * (ny + 1) + (j + 1) * (nx + 1);
                int n3_id = (k + 1) * (nx + 1) * (ny + 1) + j * (nx + 1);
                left_bnd->addElement(std::make_unique<QuadElement>(face_id_counter++, std::vector<Node*>{
                    geometry->getMesh().getNodeById(n0_id),
                    geometry->getMesh().getNodeById(n1_id),
                    geometry->getMesh().getNodeById(n2_id),
                    geometry->getMesh().getNodeById(n3_id)
                }));

                // right face (x=width) - 创建四边形面单元
                n0_id = k * (nx + 1) * (ny + 1) + j * (nx + 1) + nx;
                n1_id = k * (nx + 1) * (ny + 1) + (j + 1) * (nx + 1) + nx;
                n2_id = (k + 1) * (nx + 1) * (ny + 1) + (j + 1) * (nx + 1) + nx;
                n3_id = (k + 1) * (nx + 1) * (ny + 1) + j * (nx + 1) + nx;
                right_bnd->addElement(std::make_unique<QuadElement>(face_id_counter++, std::vector<Node*>{
                    geometry->getMesh().getNodeById(n0_id),
                    geometry->getMesh().getNodeById(n1_id),
                    geometry->getMesh().getNodeById(n2_id),
                    geometry->getMesh().getNodeById(n3_id)
        ...
        for (int k = 0; k < nz; ++k) {
            for (int i = 0; i < nx; ++i) {
                // front face (y=0) - 创建四边形面单元
                int n0_id = k * (nx + 1) * (ny + 1) + i;
                int n1_id = k * (nx + 1) * (ny + 1) + i + 1;
                int n2_id = (k + 1) * (nx + 1) * (ny + 1) + i + 1;
                int n3_id = (k + 1) * (nx + 1) * (ny + 1) + i;
                front_bnd->addElement(std::make_unique<QuadElement>(face_id_counter++, std::vector<Node*>{
                    geometry->getMesh().getNodeById(n0_id),
                    geometry->getMesh().getNodeById(n1_id),
                    geometry->getMesh().getNodeById(n2_id),
                    geometry->getMesh().getNodeById(n3_id)
                ...
                // back face (y=height) - 创建四边形面单元
                n0_id = k * (nx + 1) * (ny + 1) + ny * (nx + 1) + i;
                n1_id = k * (nx + 1) * (ny + 1) + ny * (nx + 1) + i + 1;
                n2_id = (k + 1) * (nx + 1) * (ny + 1) + ny * (nx + 1) + i + 1;
                n3_id = (k + 1) * (nx + 1) * (ny + 1) + ny * (nx + 1) + i;
                back_bnd->addElement(std::make_unique<QuadElement>(face_id_counter++, std::vector<Node*>{
                    geometry->getMesh().getNodeById(n0_id),
                    geometry->getMesh().getNodeById(n1_id),
                    geometry->getMesh().getNodeById(n2_id),
                    geometry->getMesh().getNodeById(n3_id)
        ...
        for (int j = 0; j < ny; ++j) {
            for (int i = 0; i < nx; ++i) {
                // bottom face (z=0) - 创建四边形面单元
                int n0_id = j * (nx + 1) + i;
                int n1_id = j * (nx + 1) + i + 1;
                int n2_id = (j + 1) * (nx + 1) + i + 1;
                int n3_id = (j + 1) * (nx + 1) + i;
                bottom_bnd->addElement(std::make_unique<QuadElement>(face_id_counter++, std::vector<Node*>{
                    geometry->getMesh().getNodeById(n0_id),
                    geometry->getMesh().getNodeById(n1_id),
                    geometry->getMesh().getNodeById(n2_id),
                    geometry->getMesh().getNodeById(n3_id)
                ...
                // top face (z=depth) - 创建四边形面单元
                n0_id = nz * (nx + 1) * (ny + 1) + j * (nx + 1) + i;
                n1_id = nz * (nx + 1) * (ny + 1) + j * (nx + 1) + i + 1;
                n2_id = nz * (nx + 1) * (ny + 1) + (j + 1) * (nx + 1) + i + 1;
                n3_id = nz * (nx + 1) * (ny + 1) + (j + 1) * (nx + 1) + i;
                top_bnd->addElement(std::make_unique<QuadElement>(face_id_counter++, std::vector<Node*>{
                    geometry->getMesh().getNodeById(n0_id),
                    geometry->getMesh().getNodeById(n1_id),
                    geometry->getMesh().getNodeById(n2_id),
                    geometry->getMesh().getNodeById(n3_id)
